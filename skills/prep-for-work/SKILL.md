...
2. **query-check** (refuses): any query used as evidence — a kill criterion, a recurrence count, a "returns zero so we're clean" — must carry a `window` and `expectAtLeast` naming a known-positive history, and the tool RUNS it. Motivating case: a kill criterion's grep matched **2 of the 6 commits its own plan cited** — it would have returned zero and auto-selected "delete the tool" while the problem recurred.
   **Two things decide the verdict, and the exit status is the first of them.** A run that exits nonzero or is killed is a FAILED run, not a small count: it is stamped `observed: {count: 0, failed: true}` and `validate` refuses it, so output alone can never buy a pass. (Until 2026-07-30 only spawn failure was noticed — spawnSync sets `error` when the *shell* cannot start, never when the command it ran exits 1 — so a query printing three lines and exiting 1 was stamped `ok`, and the canonical zero-match `… | grep -c 'fix('` exited 1 with stdout `0`, which read as `ok 1 ≥ 1`: the kill criterion returning zero, passing.) The second is **how the output is counted**: a pipeline whose LAST stage is `grep -c` / `rg -c` / `wc -l` — reached by a **pipe**, or standing alone as the whole query — is counted by NUMBER when stdout is a single bare integer; everything else counts non-empty stdout lines. Both halves are required. **A counting pipeline whose stdout is not a number is REFUSED, not line-counted (DER-2841)** — `grep -c`/`rg -c` over *multiple* files emit `path:count` rows, and the old fallback counted those ROWS, i.e. the number of files SEARCHED: a file matching **zero** times counted as a match, so `grep -c 'x' a.txt b.txt c.txt` with one real match in `a.txt` was stamped `3 ≥ 3`. The inflation is exactly the number of files searched, so a wider search produced a more convincing wrong number. Count the matching LINES instead — `grep PATTERN a.txt b.txt | wc -l`, dropping the `-c` — which is what a match count is. **Do NOT append `| wc -l` while KEEPING `-c`**: that counts the ROWS of a count (the number of files), and over a single file it answers 1 for every pattern, matching or not (DER-2900). A **prefixed single row is refused too**, and the message names the spelling that works per command: `wc -l FILE` prints `COUNT path` so use `wc -l < FILE`; a prefixed `grep -Hc`/`rg -c` should drop the `-H` and name one file so stdout is a bare number. An earlier version of this fix read one `path:count` row as its number and was REVERTED: `/^(.*):(\d+)$/` is greedy and has no notion of "count", so it matched any line ending in `:digits` — `wc -l 'notes:2026'` printed `3 notes:2026` and was read as **2026**, passing a floor of 2000 against a true count of 3. A fail-closed wrong answer beats a fail-open fabricated one. A `;`/`&&`/`||`-joined trailing `wc -l` consumed nothing upstream so its number is not the query's answer. **A trailing `|| true` / `; true` / `| cat` / `| tee` is refused outright (DER-2810)**: none can add information to an evidence query and each can subtract it — `… | cat` moves the counting command off the end of the pipeline so a count of `0` reads as one line and clears a floor of 1. **A query beginning with a bare separator (`| wc -l`) is refused (DER-2808)**, naming the operator. Write the documented shape — `git log --oneline --since=… | grep -c 'fix('` with `expectAtLeast: 6` — and it now means what it says; before numeric mode that floor was unsatisfiable at any number of matches, because `grep -c` prints one line whether it counted 5 or 500.
