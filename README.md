...
| **Codex CLI** | The **default pre-PR review gate on the first review round** (the Claude 3-lens panel runs beside it as the backup, and is the whole gate on later rounds and whenever codex is unavailable) **and** the mandatory plan review. Both degrade gracefully when absent, and both skips must be written down: `codex-probe` prints the review waiver line to paste, and a skipped plan review needs `planReviewSkipped`. | [github.com/openai/codex](https://github.com/openai/codex) · then `codex login` |
